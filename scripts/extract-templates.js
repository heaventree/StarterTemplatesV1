...
const templateItems = document.querySelectorAll('.ast-col-md-4');
templateItems.forEach((item, index) => {
  try {
    // Get image URL
    const imageElement = item.querySelector('img[class*="attachment-"]');
    const imageUrl = imageElement ? imageElement.getAttribute('src') : null;
    
    // Get template title
    const titleElement = item.querySelector('.ast-site-title h3');
    const title = titleElement ? titleElement.textContent.trim() : `Template ${index + 1}`;
    
    // Get demo URL
    const demoLinkElement = item.querySelector('a.elementor-button-link, a.demo-link, a[href*="websitedemos"]');
    const demoUrl = demoLinkElement ? demoLinkElement.getAttribute('href') : null;
    
    // Get category info
    const categoryElement = item.querySelector('.ast-site-cate');
    const category = categoryElement ? categoryElement.textContent.trim() : 'Other';
    
    // Get page builder info (if available)
    const builderElement = item.querySelector('.ast-sites__type');
    const pageBuilder = builderElement ? builderElement.textContent.trim() : 'Elementor';
    
    // Get pro/free status
    const isPro = !!item.querySelector('.ast-tooltip');
    
    // Determine tags
    const tags = [];
    if (category) tags.push(category);
    if (pageBuilder) tags.push(pageBuilder);
